...
def post_language_model_processing(lm_output, labels, logit_weights,
    ...
    if labels is None:
        ...
        # [b s] => [s b]
        labels = paddle.transpose(labels, perm=[1, 0])
        if fp16_lm_cross_entropy:
            assert output.dtype == paddle.float16
            loss = tensor_parallel.vocab_parallel_cross_entropy(output, labels)
        else:
            loss = tensor_parallel.vocab_parallel_cross_entropy(output.astype(paddle.float32), labels)
        
        # [s b] => [b, s]
        loss = paddle.transpose(loss, perm=[1, 0])
        return loss
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.post_process and not self.pre_process and not self.untie_embeddings_and_output_weights:
            self.word_embeddings.set_state_dict(
                state_dict[self._word_embeddings_for_head_key])
        if self._language_model_key in state_dict:
            state_dict = state_dict[self._language_model_key]
        self.language_model.set_state_dict(state_dict) 